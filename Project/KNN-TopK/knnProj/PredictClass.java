...
public class PredictClass {
	...
	public static class KNN_Multi_Mapper extends Mapper<Object, Text, Text, DistClass> {
		...
		// To store local Top K (distance, class) for all test records
		...
		public void map(final Object key, final Text value, final Context context)
			...
			for (TestData td : testDataList) {
				...
				if (localTopK.containsKey(td.getTestIdentifier())) {
					localTopK.get(td.getTestIdentifier()).put(distance, train_class);
				} else {
					TreeMap<Double, String> tm = new TreeMap<Double, String>();
					tm.put(distance, train_class);
					localTopK.put(td.getTestIdentifier(), tm);
				}

				if (localTopK.get(td.getTestIdentifier()).size() > k) {
					localTopK.get(td.getTestIdentifier()).pollLastEntry();
				}
			}
		}
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			for (Map.Entry<String, TreeMap<Double, String>> entry : localTopK.entrySet()) {
				for (Map.Entry<Double, String> treeEntry : entry.getValue().entrySet()) {
					DistClass distanceClassifier = new DistClass();
					distanceClassifier.set(treeEntry.getKey(), treeEntry.getValue());
					context.write(new Text(entry.getKey()), distanceClassifier);
				}
			}
		}
